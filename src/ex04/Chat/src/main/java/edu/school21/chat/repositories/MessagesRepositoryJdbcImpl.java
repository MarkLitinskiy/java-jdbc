...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public List<User> findAll(int page, int size) throws SQLException {
        ...
        String query = "select users.id as users_id, user_login, user_password, list_of_created_rooms.id, chatroom_owner_id, chatroom_id, chatroom_name from chat_database.users as users " +
                "join chat_database.list_of_created_rooms as list_of_created_rooms on users.id = list_of_created_rooms.chatroom_owner_id " +
                "order by users.id " +
                "LIMIT " + size + " OFFSET " + page * size;
        try (Statement statementOut = connection.createStatement();
        ...
        {
            ResultSet resultSet = statementOut.executeQuery(query);
            while(resultSet.next()){
                ResultSet resultSetOfCreatedRooms = statementOfCreatedRooms.executeQuery(query);
                ResultSet resultSetOfUserSocialized = statementOfUserSocialized.executeQuery(query);
                usersOut.add(new User(resultSet.getLong("users_id"),
                        resultSet.getString("user_login"),
                        resultSet.getString("user_password"),
                        findAllChatsCreated(resultSetOfCreatedRooms),
                        findAllUsersSocialized(resultSetOfUserSocialized)));
            }
            return usersOut;
        } catch (SQLException exception){
            throw new SQLException(exception);
    ...
    private List<Chat> findAllUsersSocialized(ResultSet resultSet) throws SQLException {
        List<Chat> allChatRooms = new ArrayList<>();
        while(resultSet.next()){
            allChatRooms.add(new Chat(resultSet.getLong("chatroom_id"),
    ...
    private List<Chat> findAllChatsCreated(ResultSet resultSet) throws SQLException {
        List<Chat> chatRoomsCreated = new ArrayList<>();
        while(resultSet.next()) {
            if(resultSet.getLong("chatroom_owner_id") == resultSet.getLong("users_id")){
                chatRoomsCreated.add(new Chat(resultSet.getLong("chatroom_id"),
                        resultSet.getString("chatroom_name"),
                        resultSet.getLong("chatroom_owner_id"),
                        new ArrayList<>()));
            }
